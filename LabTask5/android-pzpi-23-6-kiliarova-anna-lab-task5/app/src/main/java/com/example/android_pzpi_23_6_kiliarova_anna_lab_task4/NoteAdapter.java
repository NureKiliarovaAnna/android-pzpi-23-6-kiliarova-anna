...
public class NoteAdapter extends RecyclerView.Adapter<NoteAdapter.NoteViewHolder> {
    ...
    private float textSize; // Розмір шрифту

    public NoteAdapter(List<Note> notes, Context context, float textSize) {
        this.notes = notes;
        this.context = context;
        this.notesFull = new ArrayList<>(notes); // Копія для оригінального списку
        this.textSize = textSize; // Передаємо розмір шрифту
    }
    ...
    public void onBindViewHolder(@NonNull NoteViewHolder holder, int position) {
        ...
        holder.tvTitle.setTextSize(textSize); // Застосовуємо розмір шрифту для заголовка
        holder.tvDateTime.setText(note.getDateTime());
        holder.tvDateTime.setTextSize(textSize - 2); // Трохи менший розмір для дати
    ...
    // Динамічне оновлення розміру шрифту
